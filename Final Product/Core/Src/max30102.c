  ...
  while (i < size - 1)
  {
    if (x[i] > th && x[i] > x[i - 1])
    {
      ...
      while (i + n_width < size - 1 && x[i] == x[i + n_width])
        n_width++;
      if (x[i] > x[i + n_width] && n_peaks < max)
      {
        ir_valley_locs[n_peaks] = i;
        int k = n_peaks - 1;
        while (k >= 0 && i > ir_valley_locs[k])
        {
...
int removeclose(int n_peaks, int rdata[], int min)
{
  ...
  for (int i = 0; i < n_peaks - 1; i++)
    for (int j = i + 1; j < n_peaks; j++)
      ...
      {
        int temp = ir_valley_locs[i];
        ir_valley_locs[i] = ir_valley_locs[j];
        ir_valley_locs[j] = temp;
      }
}
int calc(int rdata[])
{
  ...
  for (int i = 0; i < amount - 4; i++)
    rdata[i] = (rdata[i] + rdata[i + 1] + rdata[i + 2] + rdata[i + 3]) / 4;
  sum = 0;
  for (int i = 0; i < amount; i++)
    sum += rdata[i];
  mean = sum / amount;
  int th = mean < 30 ? 30 : mean > 60 ? 60
                                      : mean;
  int n_peaks = findvalid(rdata, 100, th, 15);
  int n_peaks2 = removeclose(n_peaks, rdata, 4);
  n_peaks = n_peaks < n_peaks2 ? n_peaks : n_peaks2;
  ...
  if (n_peaks >= 2)
  {
    ...
    hr = 25 * 60 / peak_interval_sum;
    prevhr = hr > 55 && hr < 110 ? hr : prevhr;
    hr = hr > 55 && hr < 120 ? hr : -999;
  }
  else
    hr = -999;
  return hr;
...
int read_sequential()
{
  ...
  for (int ii = 0; ii < 4; ii++)
  {
    for (i = 0; i < amount; i++)
    {
      ...
      {
      } /*There is an implicit assumption that GPIOA pin 5 is the interrupt line*/
      read_fifo();
